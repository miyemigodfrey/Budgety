...
import type { Source, Transaction } from "@prisma/client";
...
/**
 * Ported verbatim from the NestJS pdf.util, except amounts are BigInt minor
 * units now, so they pass through toMajor before Intl formatting.
 *
 * Requires the Node runtime — pdfkit uses fs/stream/Buffer and reads its AFM
 * font metrics from disk. The route handler that calls this sets
...
export function generateTransactionsPdf(options: PdfOptions): Promise<Buffer> {
	...
	return new Promise((resolve, reject) => {
		...
		for (const tx of transactions) {
			if (tx.type === "inflow") totalInflow += tx.amount;
			if (tx.type === "outflow") totalOutflow += tx.amount;
		}
		...
		for (const tx of transactions) {
			...
			const targetName = tx.transferTargetId
				...
				: "";
			const sourceLabel =
				tx.type === "transfer" ? `${sourceName} → ${targetName}` : sourceName;
...
function formatCurrency(minor: bigint): string {
